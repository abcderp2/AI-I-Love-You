...
- APIを使用しません
- ビルド処理を必要としません
- 外部ライブラリ、外部フォント、広告、解析タグ、第三者素材を使用しません
- Cookie、フォーム、アカウント機能を使用しません
- サイトコードによる個人情報の意図的な収集を行いません
- 人、支援技術、検索クローラー、AIが読みやすいセマンティックHTMLを使用します
- 通常のコード編集能力を持つAIが理解し、保守できる小さなファイル構成です
- GitHub Pagesで追加の有料サービスを使わず公開できます
...
サイト自身には、クライアント側の追跡コード、Cookie、解析タグ、外部サービスへの通信処理がありません。ただし、配信基盤であるGitHub Pagesでは、GitHubがサービス運用上の情報を独自の規約と方針に基づいて処理する場合があります。
...
GitHubリポジトリのSettingsからPagesを開き、mainブランチのrootディレクトリから公開する設定を選びます。設定後、上記の公開URLで配信されます。

### 保守方法

HTML、CSS、Markdown、テキストファイルを直接編集します。パッケージのインストール、秘密情報、APIキー、有料サービス、生成済み画像は不要です。
...
- No API
- No build process
- No external libraries, fonts, advertisements, analytics, or third-party assets
- No cookies, forms, or accounts
- No intentional collection of personal information by the site code
- Semantic HTML readable by people, assistive technology, crawlers, and AI systems
- Small files that ordinary coding assistants can understand and maintain
- Free publication through GitHub Pages without an additional paid service
...
The site itself contains no client-side tracking code, cookies, analytics, or requests to third-party services. GitHub Pages is the hosting provider and may independently process operational information under GitHub's own terms and policies.

### Files

- index.html: complete page and story
...
Open Pages in the repository settings and select deployment from the root directory of the main branch. GitHub will publish the site at the URL above.

### Maintenance

Edit the plain HTML, CSS, Markdown, or text files directly. No package installation, secret, API key, paid service, or generated image is required.
